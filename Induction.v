...
Proof.
  intros n m. induction n as [| n' IHn'].
  - (* n = 0 *) rewrite -> plus_O_n. rewrite <- plus_n_O. reflexivity.
...
(** **** Exercise: 2 stars, advanced, especially useful (plus_comm_informal)
    ...
    Theorem: Addition is commutative.forall m, n : nat , m + n = n + m
    By induction on [n].

    - First [ n = 0 ]. We must show

       m + 0 = 0 + m

     By the defintion of [+] and rules plus_n_0 we get

      m = 0 + m

    By the defintion of [+] and rules plus_0_n we get
   ...
   - Next, suppose [n = S n'], where
    ...
    By plus_assoc we get
     1 + (n' + m) = 1 + (n' + m)

    and by the definition of [+] the equation holds true. QED
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)
    ...
    Proof:
     ...
     - First let [n = 0]. We now have to show
        true = 0 =? 0
       which holds.
     ...
     - Next let [n = S n'] and
       ...
       by IH we know that [n' = n'] and by [+] we can deduce
        true = 1 + n' =? 1 + n'
 *)
(** [] *)
...
Theorem plus_swap : forall n m p : nat,
    n + (m + p) = m + (n + p).
Proof.
  intros n m p. assert (H: n + m = m + n).
  { rewrite -> plus_comm. reflexivity. }
  rewrite -> plus_assoc. rewrite -> plus_assoc. rewrite -> H. reflexivity.  Qed.

(** Now prove commutativity of multiplication.  You will probably
    want to define and prove a "helper" theorem to be used
    in the proof of this one. Hint: what is [n * (1 + k)]? *)
(*
Theorem mult_dist: forall n m : nat,
  m * S n = m + n * m.
Proof.
  intros n m. induction n as [| n' IHn'].
  * simpl. rewrite -> mult_n_1. rewrite <- plus_n_O. reflexivity.
  * simpl. rewrite <- IHn'. rewrite <- plus_1_l. rewrite -> mult *)
...
Proof.
  ...
  * simpl. assert (H: m * (1 + n') = m + n' * m).
    { simpl. rewrite <- plus_comm. rewrite <- mult_n_Sm.
      rewrite -> IHn'. reflexivity. }
    rewrite <- plus_1_l. rewrite -> H. reflexivity.   Qed.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. reflexivity.
  - simpl. rewrite IHn. simpl. rewrite <- plus_n_O. rewrite <- plus_n_Sm. reflexivity.
Qed.
...
(*
  ...
  There are multiple binary representations for 0, 0, 00, 00000,
  000000, etc.  so starting from 0000 we get 0 which though different
  representations they denote the same value.
...
Fixpoint normalize (n: bin) : bin :=
  match n with
  | Z => Z
  | B0 n' => if (is_zero n') then Z else B0 (normalize n')
  | B1 n' => B1 (normalize n')
  end.
